package a.a.a.c;

import com.tencent.mm.bd.b;
import java.io.OutputStream;
import java.util.LinkedList;

public final class a
{
  public final OutputStream hrr;
  public final byte[] xkE;
  private final a.a.a.b.b.a xkF;
  
  public a(byte[] paramArrayOfByte)
  {
    this.xkE = paramArrayOfByte;
    this.hrr = null;
    this.xkF = new a.a.a.b.b.a(paramArrayOfByte, 0, paramArrayOfByte.length);
  }
  
  public final void O(int paramInt, long paramLong)
  {
    a.a.a.b.b.a locala = this.xkF;
    locala.at(paramInt, 0);
    locala.n(paramLong);
  }
  
  public final void a(int paramInt, double paramDouble)
  {
    a.a.a.b.b.a locala = this.xkF;
    locala.at(paramInt, 1);
    long l = Double.doubleToLongBits(paramDouble);
    locala.cG((int)l & 0xFF);
    locala.cG((int)(l >> 8) & 0xFF);
    locala.cG((int)(l >> 16) & 0xFF);
    locala.cG((int)(l >> 24) & 0xFF);
    locala.cG((int)(l >> 32) & 0xFF);
    locala.cG((int)(l >> 40) & 0xFF);
    locala.cG((int)(l >> 48) & 0xFF);
    locala.cG((int)(l >> 56) & 0xFF);
  }
  
  public final void an(int paramInt, boolean paramBoolean)
  {
    int i = 0;
    a.a.a.b.b.a locala = this.xkF;
    locala.at(paramInt, 0);
    paramInt = i;
    if (paramBoolean) {
      paramInt = 1;
    }
    locala.cG(paramInt);
  }
  
  public final void b(int paramInt, b paramb)
  {
    a.a.a.b.b.a locala = this.xkF;
    if (paramb != null)
    {
      locala.at(paramInt, 2);
      paramb = paramb.toByteArray();
      locala.cI(paramb.length);
      locala.j(paramb);
    }
  }
  
  public final void c(int paramInt, LinkedList<?> paramLinkedList)
  {
    int j = 0;
    a.a.a.b.b.a locala;
    int i;
    if ((paramLinkedList != null) && (paramLinkedList.size() > 0))
    {
      this.xkF.at(paramInt, 2);
      locala = this.xkF;
      if ((paramLinkedList == null) || (paramLinkedList.size() <= 0)) {
        break label123;
      }
      paramInt = 0;
      i = 0;
      if (i < paramLinkedList.size()) {
        break label70;
      }
    }
    for (;;)
    {
      locala.cI(paramInt);
      paramInt = j;
      for (;;)
      {
        if (paramInt >= paramLinkedList.size())
        {
          return;
          label70:
          int k = a.a.a.b.b.a.cJ(((Integer)paramLinkedList.get(i)).intValue());
          i += 1;
          paramInt = k + paramInt;
          break;
        }
        this.xkF.cI(((Integer)paramLinkedList.get(paramInt)).intValue());
        paramInt += 1;
      }
      label123:
      paramInt = 0;
    }
  }
  
  public final void d(int paramInt1, int paramInt2, LinkedList<?> paramLinkedList)
  {
    if (paramLinkedList != null) {
      switch (paramInt2)
      {
      default: 
        throw new IllegalArgumentException("The data type was not found, the id used was " + paramInt2);
      case 6: 
        paramInt2 = 0;
        if (paramInt2 < paramLinkedList.size()) {
          break;
        }
      }
    }
    for (;;)
    {
      return;
      b(paramInt1, (b)paramLinkedList.get(paramInt2));
      paramInt2 += 1;
      break;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        a(paramInt1, ((Double)paramLinkedList.get(paramInt2)).doubleValue());
        paramInt2 += 1;
      }
      continue;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        n(paramInt1, ((Float)paramLinkedList.get(paramInt2)).floatValue());
        paramInt2 += 1;
      }
      continue;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        eO(paramInt1, ((Integer)paramLinkedList.get(paramInt2)).intValue());
        paramInt2 += 1;
      }
      continue;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        O(paramInt1, ((Long)paramLinkedList.get(paramInt2)).longValue());
        paramInt2 += 1;
      }
      continue;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        e(paramInt1, (String)paramLinkedList.get(paramInt2));
        paramInt2 += 1;
      }
      continue;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        an(paramInt1, ((Boolean)paramLinkedList.get(paramInt2)).booleanValue());
        paramInt2 += 1;
      }
      continue;
      paramInt2 = 0;
      while (paramInt2 < paramLinkedList.size())
      {
        com.tencent.mm.bd.a locala = (com.tencent.mm.bd.a)paramLinkedList.get(paramInt2);
        eQ(paramInt1, locala.aUd());
        locala.a(this);
        paramInt2 += 1;
      }
    }
  }
  
  public final void e(int paramInt, String paramString)
  {
    a.a.a.b.b.a locala = this.xkF;
    if (paramString != null)
    {
      locala.at(paramInt, 2);
      paramString = paramString.getBytes("UTF-8");
      locala.cI(paramString.length);
      locala.j(paramString);
    }
  }
  
  public final void eO(int paramInt1, int paramInt2)
  {
    a.a.a.b.b.a locala = this.xkF;
    locala.at(paramInt1, 0);
    if (paramInt2 >= 0)
    {
      locala.cI(paramInt2);
      return;
    }
    locala.n(paramInt2);
  }
  
  public final void eP(int paramInt1, int paramInt2)
  {
    eO(paramInt1, paramInt2);
  }
  
  public final void eQ(int paramInt1, int paramInt2)
  {
    a.a.a.b.b.a locala = this.xkF;
    locala.at(paramInt1, 2);
    locala.cI(paramInt2);
  }
  
  public final void n(int paramInt, float paramFloat)
  {
    a.a.a.b.b.a locala = this.xkF;
    locala.at(paramInt, 5);
    paramInt = Float.floatToIntBits(paramFloat);
    locala.cG(paramInt & 0xFF);
    locala.cG(paramInt >> 8 & 0xFF);
    locala.cG(paramInt >> 16 & 0xFF);
    locala.cG(paramInt >> 24 & 0xFF);
  }
}


/* Location:              /home/chengjian/weinxin/weinxin_1100/classes.jar!/a/a/a/c/a.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */