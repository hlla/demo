...
{
  private static String xgM = null;
  private final Context mContext;
  public final HashMap<String, String> xgN;
  private final HashMap<String, String> xgO;
  
  public ShareSecurityCheck(Context paramContext)
  {
    this.mContext = paramContext;
    this.xgN = new HashMap();
    this.xgO = new HashMap();
    if (xgM == null)
    {
      paramContext = this.mContext;
      try
      {
        paramContext = SharePatchFileUtil.bG(paramContext.getPackageManager().getPackageInfo(paramContext.getPackageName(), 64).signatures[0].toByteArray());
        xgM = paramContext;
  ...
  private static boolean a(File paramFile, Certificate[] paramArrayOfCertificate)
  {
    if (paramArrayOfCertificate.length > 0)
    {
      ...
      while (i >= 0) {
        try
        {
          boolean bool = xgM.equals(SharePatchFileUtil.bG(paramArrayOfCertificate[i].getEncoded()));
          if (bool) {
            return true;
          }
        }
  ...
  /* Error */
  public final boolean aa(File paramFile)
  {
    // Byte code:
    //   0: aload_1
    //   1: invokestatic 107	com/tencent/tinker/loader/shareutil/SharePatchFileUtil:U	(Ljava/io/File;)Z
    //   4: ifne +5 -> 9
    //   7: iconst_0
    //   8: ireturn
    ...
    //   74: ifeq -49 -> 25
    //   77: aload_0
    //   78: getfield 30	com/tencent/tinker/loader/shareutil/ShareSecurityCheck:xgN	Ljava/util/HashMap;
    //   81: aload 6
    //   83: aload 4
    //   85: aload 5
  ...
  public final HashMap<String, String> cgJ()
  {
    if (!this.xgO.isEmpty()) {
      return this.xgO;
    }
    Object localObject = (String)this.xgN.get("assets/package_meta.txt");
    if (localObject == null) {
      return null;
    }
    ...
    while (i < j)
    {
      ...
      if ((arrayOfString != null) && (arrayOfString.length() > 0) && (!arrayOfString.startsWith("#")))
      {
        ...
        if ((arrayOfString != null) && (arrayOfString.length >= 2)) {
          this.xgO.put(arrayOfString[0].trim(), arrayOfString[1].trim());
        }
      }
      i += 1;
    }
...
/* Location:              /home/chengjian/weinxin/weinxin_1100/classes.jar!/com/tencent/tinker/loader/shareutil/ShareSecurityCheck.class
