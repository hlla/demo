...
public final class WWAPIImpl
  implements IWWAPI
{
  private static final String[] xjO = { "com.tencent.wework", "com.tencent.weworklocal" };
  private Context context;
  public Map<String, Object> pQL = new HashMap();
  public String xjP;
  private BroadcastReceiver xjQ = new BroadcastReceiver()
  {
    public void onReceive(final Context paramAnonymousContext, Intent paramAnonymousIntent)
    {
      try
      {
        if (!WWAPIImpl.this.xjP.equals(paramAnonymousIntent.getScheme()))
        {
        ...
        if ((paramAnonymousContext instanceof WWBaseRespMessage))
        {
          new Handler(Looper.getMainLooper()).post(new Runnable()
          {
            public void run()
            {
              try
              {
                WWAPIImpl.this.pQL.get(((WWBaseRespMessage)paramAnonymousContext).transaction);
                WWAPIImpl.this.pQL.remove(((WWBaseRespMessage)paramAnonymousContext).transaction);
                return;
              }
  ...
  private String Wl(String paramString)
  {
    try
    {
      String str = bI(this.context.getPackageManager().getPackageInfo(paramString, 64).signatures[0].toByteArray());
      return str;
    }
    catch (Throwable localThrowable)
    {
      Log.b("WWAPIImpl", "getSignature failed, pkg: " + paramString, localThrowable);
    }
    return "";
  }
  
  private int Wm(String paramString)
  {
    try
    {
    ...
    {
      Log.b("WWAPIImpl", "getVersioncode failed, pkg: " + paramString, localThrowable);
    }
    return 0;
  }
  
  private static String bI(byte[] paramArrayOfByte)
  {
    try
    {
    ...
    for (;;)
    {
      ...
      else
      {
        try
        {
          ...
          ((Intent)localObject).putExtra("PendingIntent", PendingIntent.getBroadcast(this.context, 0, new Intent(this.context, this.xjQ.getClass()), 134217728));
        ...
        {
          Log.b("WWAPIImpl", "sendMessage failed, pkg: " + str, localThrowable);
        }
      }
      i += 1;
    }
  ...
  public final String chI()
  {
    ...
    while (i < j)
    {
      ...
      {
        Log.b("WWAPIImpl", "getWWAppName failed", localThrowable);
        i += 1;
      }
    }
    return "企业微信";
...
/* Location:              /home/chengjian/weinxin/weinxin_1100/classes.jar!/com/tencent/wework/api/WWAPIImpl.class
