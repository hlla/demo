package com.tencent.mm.sdk.platformtools;

import com.tencent.gmtrace.GMTrace;
import java.util.Iterator;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public final class am
{
  public static an usN;
  
  static
  {
    GMTrace.i(13948711600128L, 103926);
    usN = null;
    GMTrace.o(13948711600128L, 103926);
  }
  
  public am()
  {
    GMTrace.i(13947637858304L, 103918);
    GMTrace.o(13947637858304L, 103918);
  }
  
  public static String PA(String paramString)
  {
    GMTrace.i(13948174729216L, 103922);
    if (bf.mA(paramString))
    {
      GMTrace.o(13948174729216L, 103922);
      return "";
    }
    paramString = paramString.replaceAll("[\\.\\-\\ ]", "").trim();
    GMTrace.o(13948174729216L, 103922);
    return paramString;
  }
  
  public static String PB(String paramString)
  {
    GMTrace.i(17568295288832L, 130894);
    if (bf.mA(paramString))
    {
      GMTrace.o(17568295288832L, 130894);
      return "";
    }
    paramString = paramString.replace("+", "");
    GMTrace.o(17568295288832L, 130894);
    return paramString;
  }
  
  public static String PC(String paramString)
  {
    GMTrace.i(17568429506560L, 130895);
    if (bf.mA(paramString))
    {
      GMTrace.o(17568429506560L, 130895);
      return "";
    }
    if (paramString.startsWith("+"))
    {
      GMTrace.o(17568429506560L, 130895);
      return paramString;
    }
    paramString = "+" + paramString;
    GMTrace.o(17568429506560L, 130895);
    return paramString;
  }
  
  public static String Pz(String paramString)
  {
    GMTrace.i(13948040511488L, 103921);
    if (bf.mA(paramString))
    {
      GMTrace.o(13948040511488L, 103921);
      return paramString;
    }
    if (!bf.Qg(paramString).booleanValue())
    {
      GMTrace.o(13948040511488L, 103921);
      return paramString;
    }
    new am();
    String str1 = "86";
    Object localObject = paramString;
    if (paramString.startsWith("+"))
    {
      String str2 = paramString.replace("+", "");
      paramString = ey(str2, null);
      str1 = paramString;
      localObject = str2;
      if (paramString != null)
      {
        localObject = str2.substring(paramString.length());
        str1 = paramString;
      }
    }
    paramString = formatNumber(str1, (String)localObject);
    GMTrace.o(13948040511488L, 103921);
    return paramString;
  }
  
  private static String Z(String paramString1, String paramString2, String paramString3)
  {
    GMTrace.i(13948577382400L, 103925);
    paramString1 = Pattern.compile(paramString1).matcher(paramString3);
    if (paramString1.find())
    {
      paramString1 = paramString1.replaceAll(paramString2);
      GMTrace.o(13948577382400L, 103925);
      return paramString1;
    }
    GMTrace.o(13948577382400L, 103925);
    return paramString3;
  }
  
  private static int cB(String paramString, int paramInt)
  {
    GMTrace.i(13948443164672L, 103924);
    Pattern localPattern = Pattern.compile(paramString);
    paramString = "1";
    int i = 0;
    while ((i < paramInt) && (!localPattern.matcher(paramString).find()))
    {
      paramString = paramString + "1";
      i += 1;
    }
    GMTrace.o(13948443164672L, 103924);
    return i + 1;
  }
  
  public static String ey(String paramString1, String paramString2)
  {
    GMTrace.i(13947772076032L, 103919);
    paramString1 = PA(paramString1).replace("+", "");
    if (usN == null) {
      usN = new an();
    }
    int i;
    if (bf.mA(paramString2))
    {
      paramString2 = usN.usO.iterator();
      do
      {
        do
        {
          if (!paramString2.hasNext()) {
            break;
          }
          localObject = (an.a)paramString2.next();
        } while (!paramString1.startsWith(((an.a)localObject).usQ));
        i = paramString1.length() - ((an.a)localObject).usQ.length();
      } while ((i < ((an.a)localObject).usR) || (i > ((an.a)localObject).usS));
      v.i("MicroMsg.PhoneFormater", "[extractCountryCode] countrycode:%s country isocode: %s country.minlen:%d country.maxlen:%d", new Object[] { ((an.a)localObject).usQ, ((an.a)localObject).usP, Integer.valueOf(((an.a)localObject).usS), Integer.valueOf(((an.a)localObject).usS) });
      paramString1 = ((an.a)localObject).usQ;
      GMTrace.o(13947772076032L, 103919);
      return paramString1;
    }
    Object localObject = usN.usO.iterator();
    while (((Iterator)localObject).hasNext())
    {
      an.a locala = (an.a)((Iterator)localObject).next();
      if (paramString1.startsWith(locala.usQ))
      {
        i = paramString1.length() - locala.usQ.length();
        if ((i >= locala.usR) && (i <= locala.usS) && (paramString2.equalsIgnoreCase(locala.usP)))
        {
          v.i("MicroMsg.PhoneFormater", "[extractCountryCode] countrycode:%s country isocode: %s country.minlen:%d country.maxlen:%d", new Object[] { locala.usQ, locala.usP, Integer.valueOf(locala.usS), Integer.valueOf(locala.usS) });
          paramString1 = locala.usQ;
          GMTrace.o(13947772076032L, 103919);
          return paramString1;
        }
      }
    }
    GMTrace.o(13947772076032L, 103919);
    return null;
  }
  
  public static String ez(String paramString1, String paramString2)
  {
    GMTrace.i(17568563724288L, 130896);
    if ((bf.mA(paramString1)) || (bf.mA(paramString2)))
    {
      GMTrace.o(17568563724288L, 130896);
      return "";
    }
    paramString1 = paramString1 + "（+" + paramString2 + "）";
    GMTrace.o(17568563724288L, 130896);
    return paramString1;
  }
  
  public static String formatNumber(String paramString1, String paramString2)
  {
    GMTrace.i(13948308946944L, 103923);
    if ((bf.mA(paramString1)) || (bf.mA(paramString2)))
    {
      GMTrace.o(13948308946944L, 103923);
      return paramString2;
    }
    if (usN == null) {
      usN = new an();
    }
    Iterator localIterator1 = usN.usO.iterator();
    an.a locala;
    String str;
    an.b localb;
    int n;
    int k;
    label554:
    do
    {
      StringBuffer localStringBuffer;
      do
      {
        Iterator localIterator2;
        while (!localIterator2.hasNext())
        {
          do
          {
            if (!localIterator1.hasNext()) {
              break;
            }
            locala = (an.a)localIterator1.next();
          } while ((locala.usQ == null) || (!paramString1.trim().toLowerCase().equals(locala.usQ.trim().toLowerCase())) || (locala.usT == null));
          str = PA(paramString2);
          if ((str != null) && (str.length() > locala.usR))
          {
            GMTrace.o(13948308946944L, 103923);
            return str;
          }
          localIterator2 = locala.usT.iterator();
        }
        localb = (an.b)localIterator2.next();
        if (!bf.mA(localb.usU)) {
          break label554;
        }
        if (locala.usT.size() <= 1) {
          break;
        }
        localStringBuffer = new StringBuffer();
        localStringBuffer.append(str);
        m = str.length();
      } while (m > cB(localb.usW, locala.usS));
      while (localStringBuffer.toString().length() < locala.usS) {
        localStringBuffer.append("0");
      }
      paramString1 = Z(localb.usW, localb.usV, localStringBuffer.toString());
      j = 0;
      i = 0;
      while (i < paramString1.length())
      {
        n = paramString1.charAt(i);
        paramString2 = paramString1;
        if (j >= m) {
          paramString2 = paramString1.substring(0, i);
        }
        k = j;
        if (n != 32)
        {
          k = j;
          if (n != 45)
          {
            k = j;
            if (n != 12290) {
              k = j + 1;
            }
          }
        }
        i += 1;
        j = k;
        paramString1 = paramString2;
      }
      GMTrace.o(13948308946944L, 103923);
      return paramString1;
      paramString1 = new StringBuffer();
      paramString1.append(str);
      m = str.length();
      while (paramString1.toString().length() < locala.usS) {
        paramString1.append("0");
      }
      paramString1 = Z(localb.usW, localb.usV, paramString1.toString());
      j = 0;
      i = 0;
      while (i < paramString1.length())
      {
        n = paramString1.charAt(i);
        paramString2 = paramString1;
        if (j >= m) {
          paramString2 = paramString1.substring(0, i);
        }
        k = j;
        if (n != 32)
        {
          k = j;
          if (n != 45)
          {
            k = j;
            if (n != 12290) {
              k = j + 1;
            }
          }
        }
        i += 1;
        j = k;
        paramString1 = paramString2;
      }
      GMTrace.o(13948308946944L, 103923);
      return paramString1;
    } while (!Pattern.compile(localb.usU).matcher(str).lookingAt());
    paramString1 = new StringBuffer();
    paramString1.append(str);
    int m = str.length();
    while (paramString1.toString().length() < locala.usS) {
      paramString1.append(str.charAt(m - 1));
    }
    paramString1 = Z(localb.usW, localb.usV, paramString1.toString());
    int j = 0;
    int i = 0;
    while (i < paramString1.length())
    {
      n = paramString1.charAt(i);
      paramString2 = paramString1;
      if (j >= m) {
        paramString2 = paramString1.substring(0, i);
      }
      k = j;
      if (n != 32)
      {
        k = j;
        if (n != 45)
        {
          k = j;
          if (n != 12290) {
            k = j + 1;
          }
        }
      }
      i += 1;
      j = k;
      paramString1 = paramString2;
    }
    GMTrace.o(13948308946944L, 103923);
    return paramString1;
    GMTrace.o(13948308946944L, 103923);
    return paramString2;
  }
  
  public static String yS(String paramString)
  {
    GMTrace.i(13947906293760L, 103920);
    paramString = ey(paramString, null);
    GMTrace.o(13947906293760L, 103920);
    return paramString;
  }
}


/* Location:              /home/chengjian/weinxin/weinxin_1100/classes.jar!/com/tencent/mm/sdk/platformtools/am.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */