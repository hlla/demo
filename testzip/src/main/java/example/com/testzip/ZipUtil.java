...
final class ZipUtil {
    ...
    static ZipUtil.CentralDirectory findCentralDirectory(RandomAccessFile raf) throws
    ...
    static long computeCrcOfCentralDir(RandomAccessFile raf, ZipUtil.CentralDirectory dir) throws
            IOException {
        CRC32 crc = new CRC32();
        ...
        for (length = raf.read(buffer, 0, length); length != -1; length = raf.read(buffer, 0,
                length)) {
